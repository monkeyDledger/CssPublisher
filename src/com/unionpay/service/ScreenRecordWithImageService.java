...
public class ScreenRecordWithImageService extends Thread{
    ...
    public ScreenRecordWithImageService(int width, int height, String url, int bitrate, int dpi, MediaProjection mp,
	    Context context) {
	mWidth = width;
	mHeight = height;
	rtmpUrl = url;
	...
	imageReader = ImageReader.newInstance(mWidth,
		mHeight, 0x1, 2);
    }
	...
	mPublisherJni.SmartPublisherOnCaptureVideoRGBAData(buffer, rowStride,
			width, height);
}
	...
	if (encodedData != null) {
	    ...
	    Log.i(TAG, "push " + mBufferInfo.size + " bytes to muxer...");
	}
    }
